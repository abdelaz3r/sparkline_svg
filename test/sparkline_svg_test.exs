defmodule SparklineSvgTest do
  ...
  test "valid padding" do
    ...
    assert sparkline.datapoints == [
             %Datapoint{source: {1, 1}, computed: {2.0, 8.0}},
             %Datapoint{source: {2, 2}, computed: {8.0, 2.0}}
           ]
    ...
    assert sparkline.datapoints == [
             %Datapoint{source: {1, 1}, computed: {2.0, 8.0}},
             %Datapoint{source: {2, 2}, computed: {8.0, 5.0}}
           ]
    ...
    assert sparkline.datapoints == [
             %Datapoint{source: {1, 1}, computed: {3.0, 8.0}},
             %Datapoint{source: {2, 2}, computed: {7.0, 2.0}}
           ]
    ...
    assert sparkline.datapoints == [
             %Datapoint{source: {1, 1}, computed: {3.0, 8.0}},
             %Datapoint{source: {2, 2}, computed: {6.0, 1.0}}
           ]
  end
  ...
  test "valid datapoints" do
    ...
    assert sparkline.datapoints == [
             %Datapoint{source: {0, 1}, computed: {2.0, 48.0}},
             %Datapoint{source: {1, 2}, computed: {198.0, 2.0}}
           ]

    {:ok, sparkline} = SparklineSvg.new([{1, 1}, {2, 2}]) |> SparklineSvg.dry_run()

    assert sparkline.datapoints == [
             %Datapoint{source: {1, 1}, computed: {2.0, 48.0}},
             %Datapoint{source: {2, 2}, computed: {198.0, 2.0}}
           ]
    ...
    assert sparkline.datapoints == [
             %Datapoint{source: {-1, 1}, computed: {2.0, 48.0}},
             %Datapoint{source: {0, 2}, computed: {198.0, 2.0}}
           ]

    {:ok, sparkline} = SparklineSvg.new([{1.1, 1}, {1.2, 2}]) |> SparklineSvg.dry_run()

    assert sparkline.datapoints == [
             %Datapoint{source: {1.1, 1}, computed: {2.0, 48.0}},
             %Datapoint{source: {1.2, 2}, computed: {198.0, 2.0}}
           ]
    ...
    assert sparkline.datapoints == [
             %Datapoint{source: {1_704_877_202, 1}, computed: {2.0, 48.0}},
             %Datapoint{source: {1_704_877_203, 2}, computed: {198.0, 2.0}}
           ]

    now = Time.utc_now()
    now_plus_one = Time.add(now, 1, :second)
    {:ok, sparkline} = SparklineSvg.new([{now, 1}, {now_plus_one, 2}]) |> SparklineSvg.dry_run()

    assert sparkline.datapoints == [
             %Datapoint{source: {now, 1}, computed: {2.0, 48.0}},
    ...
    assert sparkline.datapoints == [
             %Datapoint{source: {~D[2024-07-28], 1}, computed: {2.0, 48.0}},
             %Datapoint{source: {~D[2024-07-29], 2}, computed: {198.0, 2.0}}
           ]

    now = DateTime.utc_now()
    now_plus_one = DateTime.utc_now() |> DateTime.add(1, :second)
    {:ok, sparkline} = SparklineSvg.new([{now, 1}, {now_plus_one, 2}]) |> SparklineSvg.dry_run()
    ...
    assert sparkline.datapoints == [
             ...
             %Datapoint{
               source: {now_plus_one, 2},
               computed: {198.0, 2.0}
             }
           ]

    now = NaiveDateTime.utc_now()
    now_plus_one = NaiveDateTime.utc_now() |> NaiveDateTime.add(1, :second)

    {:ok, sparkline} =
      SparklineSvg.new([{now, 1}, {now_plus_one, 2}]) |> SparklineSvg.dry_run()
  ...
  test "valid order datapoints sort asc" do
    ...
    assert sparkline.datapoints == [
             %Datapoint{source: {~D[2021-01-01], 2}, computed: {2.0, 2.0}},
             %Datapoint{source: {~D[2021-01-02], 1}, computed: {198.0, 48.0}}
           ]
  ...
  test "valid datapoints with sort desc" do
    ...
    assert sparkline.datapoints == [
             %Datapoint{source: {~D[2021-01-02], 1}, computed: {2.0, 48.0}},
             %Datapoint{source: {~D[2021-01-01], 2}, computed: {198.0, 2.0}}
           ]
  ...
  test "two same points" do
    ...
    assert sparkline.datapoints == [%Datapoint{source: {1, 2}, computed: {100.0, 25.0}}]
  end
  ...
  test "with only zeros as values" do
    ...
    assert sparkline.datapoints == [
             %Datapoint{source: {1, 0}, computed: {2.0, 25.0}},
             %Datapoint{source: {2, 0}, computed: {198.0, 25.0}}
           ]
  end

  test "with one value datapoints" do
    {:ok, sparkline} = SparklineSvg.new([1]) |> SparklineSvg.dry_run()
    assert sparkline.datapoints == [%Datapoint{source: {0, 1}, computed: {100.0, 25.0}}]

    {:ok, sparkline} = SparklineSvg.new([{5, 1}]) |> SparklineSvg.dry_run()
    assert sparkline.datapoints == [%Datapoint{source: {5, 1}, computed: {100.0, 25.0}}]
  end
